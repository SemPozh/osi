...
typedef struct {
    ...
    int passes;          /* количество проходов (новый параметр) */
} cache_config_t;
...
double get_hit_ratio(performance_metrics_t* metrics) {
    if (metrics->cache_hits + metrics->cache_misses == 0) return 0;
    return (double)metrics->cache_hits / (metrics->cache_hits + metrics->cache_misses) * 100.0;
}
...
void print_metrics(performance_metrics_t* metrics, cache_config_t* cache_config) {
    ...
    double hit_ratio = get_hit_ratio(metrics);
    
    printf("\n=== МЕТРИКИ ПРОИЗВОДИТЕЛЬНОСТИ ===\n");
    printf("Режим кэширования: %s\n", cache_config->type);
    if (strcmp(cache_config->type, "vtpc") == 0) {
        printf("Параметры кэша: LRU-K=%d, блоков=%d, размер блока=%d\n", 
               cache_config->lru_k, cache_config->cache_blocks, cache_config->block_size);
    }
    ...
    if (strcmp(cache_config->type, "vtpc") == 0) {
        printf("Эффективность кэша: %.2f%% (hit ratio)\n", hit_ratio);
    }
    printf("================================\n\n");
}
...
void print_usage() {
    ...
    printf("    loader [rw] [block_size] [block_count] [file] [range] [direct] [type] [cache_type] [lru_k] [cache_blocks] [passes]\n");
    printf("\n  Параметры кэширования:\n");
    printf("    cache_type: std (стандартный), direct (без кэша), vtpc (наш кэш)\n");
    printf("    lru_k: значение K для LRU-K (только для vtpc, по умолчанию %d)\n", DEFAULT_LRU_K);
    ...
    printf("    loader r 4096 100 test.dat 0-1000 off sequential vtpc 2 1000\n");
    printf("    loader r 4096 100 test.dat 0-1000 off sequential vtpc 2 1000 2  # 2 прохода\n");
}
...
int main(int argc, char *argv[]) {
    ...
    cache_config_t cache_config = {
        ...
        .passes = 1  /* по умолчанию 1 проход */
    };
    ...
    if (argc == 8) {
        ...
        if (argc >= 10) {
            cache_config.lru_k = atoi(argv[9]);
        }
        if (argc >= 11) {
            cache_config.cache_blocks = atoi(argv[10]);
        }
        /* Новый параметр: количество проходов */
        if (argc >= 12) {
            cache_config.passes = atoi(argv[11]);
        }
    } else {
    ...
    if (strcmp(cache_config.type, "vtpc") == 0) {
        /* Сначала устанавливаем параметры кэша */
        vtpc_set_lru_k(cache_config.lru_k);
        vtpc_set_cache_blocks(cache_config.cache_blocks);
        ...
        printf("Инициализирован VTPC кэш: LRU-K=%d, блоков=%d\n", 
            cache_config.lru_k, cache_config.cache_blocks);
    }
    ...
    /* Для vtpc режима обновляем метрики из статистики кэша */
    if (strcmp(cache_config.type, "vtpc") == 0) {
        metrics.cache_hits = vtpc_get_cache_hits();
        metrics.cache_misses = vtpc_get_cache_misses();
    }
